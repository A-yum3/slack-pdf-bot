...
class Table(Flowable):
    def __init__(self, data, colWidths=None, rowHeights=None, style=None,
        ...
        if not _seqCW:
            ...
            if rl_config.allowShortTableRows and isinstance(colWidths, list):
                ...
                raise ValueError("%s data error - %d columns in data but %d in column widths" % (
                self.identity(), ncols, len(colWidths)))
        if not _seqRH:
            rowHeights = nrows * [rowHeights]
        elif len(rowHeights) != nrows:
            raise ValueError(
                "%s data error - %d rows in data but %d in row heights" % (self.identity(), nrows, len(rowHeights)))
    ...
    def _drawHLines(self, start, end, weight, color, count, space):
        ...
        if count == 1:
            ...
            lf = lambda x0, y0, x1, y1, canvLine=canvLine, ws=weight + space, count=count: _multiLine(x0, x1, y0,
                                                                                                      canvLine, ws,
                                                                                                      count)
            for y in rp:
    ...
    def _cr_1_1(self, n, nRows, repeatRows, cmds, _srflMode=False):
        ...
        for c in cmds:
            ...
            if cS:
                ...
                cS = list(cS)
                self._addCommand(
                    (c[0],) + ((sc, repeatRows.index(min(cS))), (ec, repeatRows.index(max(cS)))) + tuple(c[3:]))
            if er < n: continue
            sr = max(sr - n, 0) + nrr
            er = max(er - n, 0) + nrr
            self._addCommand((c[0],) + ((sc, sr), (ec, er)) + tuple(c[3:]))
    ...
    def _drawCell(self, cellval, cellstyle, pos, size):
        ...
        if cellstyle.destination:
            # external hyperlink
            self.canv.linkRect("", cellstyle.destination, Rect=(colpos, rowpos, colpos + colwidth, rowpos + rowheight),
                               relative=1)
...
if __name__ == '__main__':
    from app.tests import old_tables_test
